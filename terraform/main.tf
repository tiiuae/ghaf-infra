...
locals {
  ...
  persistent_id = "id1${local.opts[local.conf].persistent_id}${local.shortloc}"

  # Selects builder ssh key
  use_ext_builders  = length(local.opts[local.conf].ext_builder_machines) > 0
  builder_sshkey_id = local.use_ext_builders ? "sshb-id1ext${local.shortloc}" : "sshb${local.ws}${local.shortloc}"
...
#data "azurerm_key_vault" "ghaf_devuaentest_ca" {
#  name                = "ghaf-devuaentest-ca"
