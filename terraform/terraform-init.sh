...
usage () {
    echo ""
    echo "Usage: $MYNAME [-h] [-v] [-w WORKSPACE] [-l LOCATION] [-s] [-p]"
    echo ""
    echo "Initialize ghaf-infra terraform state (-s), persistent storage (-p) or"
    echo "workspace with the given name (-w WORKSPACE). This script will not destroy"
    echo "or re-initialize anything if the initialization has already been done earlier."
    echo ""
    echo "Options:"
...
argparse () {
    ...
    if [[ -z "$OPT_s" && -z "$OPT_p" && -z "$WORKSPACE" ]]; then
        print_err "either '-w', '-s', or '-p' must be specified"
        usage; exit 1
    fi
}
...
set_env () {
    ...
    STATE_RG="ghaf-infra-state-0-${SHORTLOC}"
    STATE_ACCOUNT="ghafinfrastate0${SHORTLOC}"
    PERSISTENT_RG="ghaf-infra-persistent-0-${SHORTLOC}"
