...
in
{
  ...
  systemd.services =
    let
      # Script that runs before the agents.
      ...
      mkAgent =
        ...
        let
          ...
          jenkins-connection-script = pkgs.writeShellScript "jenkins-connect.sh" ''
            JENKINS_SECRET="$(
    ...
    in
    {
      # The setup service does not start automatically or it would 
      # fail activation of the system when the jenkins variables are unset
      # (and they always are before manual setup)
      setup-agents = {
        path = with pkgs; [ wget ];
        serviceConfig = {
          Type = "oneshot";
