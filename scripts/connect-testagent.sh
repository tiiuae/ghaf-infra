...
usage() {
	...
	echo "You must have ssh access and passwordless sudo on the testagent."
	echo ""
	echo "Options:"
	echo " -h    Print this help message"
	echo " -a    SSH address of the testagent"
	echo " -c    URL of the jenkins controller"
	echo " -v    Set the script verbosity to DEBUG"
	echo ""
	echo "Example:"
	echo ""
	echo "  $MYNAME -a testagent-release -c https://ghaf-jenkins-controller-release.northeurope.cloudapp.azure.com"
	echo ""
}
...
write_jenkins_env() {
	if ! ssh $OPT_a "\
        sudo bash -c 'echo 'CONTROLLER=$OPT_c' > /var/lib/jenkins/jenkins.env'
        sudo systemctl restart setup-agents.service
        "; then
		print_err "Failed connecting agent. check controller log."
		exit 1
	fi
}
