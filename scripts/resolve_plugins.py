...
def resolve_dependencies_recursive(resolved: dict, manifest: dict, plugin: str) -> dict:
    ...
    for dep in manifest[plugin].get("dependencies", []):
        resolved = resolve_dependencies_recursive(resolved, manifest, dep["name"])
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for name in PLUGINS:
            ...
            resolve_dependencies_recursive(resolved, manifest, name)
        ...
        output = json.dumps(list(resolved.values()), indent=2)
